//Checking of Fisrt name field js функции для валидации поля (создание HTML формы с валидацией полей на js)
function checkFirst() { // функция проверка истинности (Методы различных типов данных в js) 
    var first = document.getElementById("first").value; // почему вар или он устарел? документ элемент HTML загруженный в браузер? ИД и номер?
    var regex = /^[a-zA-Z\s]{2,15}$/; // соотвествие строки заданому шаблону от А до Я? 

    if (regex.test(first)) { // если проверка первого верна, а в чем тут проверка? Если символы попадают в этот диапозон? 
        document.getElementById("first_Check").style.color = "black"; // свойство объектов DOM модели.
        document.getElementById("first_Check").innerHTML = "Input correct :)"; // если всё ок выдает зеленый цвет если нет идёт дальше
        return true;
    }
    else {// if it's not
...
function checkPhone() { // функция проверка номера телефона 
    var phone = document.getElementById("phone").value; // Присвоение ИД
    var regex = /^\d{3}-(\d{3})-\d{3}$/; // проверяет соответсвтует строка ХХХ где Х цифра
